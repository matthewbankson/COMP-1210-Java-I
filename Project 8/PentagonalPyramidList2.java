...
//Fields
   ...
   */
   ...
   /**
   *Finds the pyramid with the shortest base edge in the array.
   *@return pyramid with shortest base edge or null
   */
   public PentagonalPyramid findPentagonalPyramidWithShortestBaseEdge() {
      if (numPP > 0) {
         PentagonalPyramid pyramid = ppArray[0];
         double shortest = ppArray[0].getBaseEdge();
         int index = 0;
         while (index < numPP) {
            if (ppArray[index].getBaseEdge() < shortest) {
               shortest = ppArray[index].getBaseEdge();
               pyramid = ppArray[index];
            }
            index++;
         }
         return pyramid;
   ...
   /**
   *Finds the pyramid with the longest base edge.
   *@return pyramid with longest base edge or null
   */
   public PentagonalPyramid findPentagonalPyramidWithLongestBaseEdge() {
      if (ppArray != null) {
         ...
         int index = 0;
         while (index < numPP) {
            if (longest < ppArray[index].getBaseEdge()) {
               longest = ppArray[index].getBaseEdge();
               pyramid = ppArray[index];
            }
            index++;
         }
   ...
   /**
   *Finds the pyramid with the smallest volume.
   *@return pyramid with smallest volume or null
   */
   public PentagonalPyramid findPentagonalPyramidWithSmallestVolume() {
      if (numPP > 0) {
         double smallest = ppArray[0].volume();
         PentagonalPyramid pyramid = ppArray[0];
         int index = 0;
         while (index < numPP) {
            if (ppArray[index].volume() < smallest) {
               smallest = ppArray[index].volume();
               pyramid = ppArray[index];
            }
            index++;
         }
   ...
   /**
   *Finds the pyramid with the largest volume.
   *@return pyramid with largest volume or null
   */
   public PentagonalPyramid findPentagonalPyramidWithLargestVolume() {
      if (ppArray != null) {
         ...
         int index = 0;
         while (index < numPP) {
            if (largest < ppArray[index].volume()) {
               largest = ppArray[index].volume();
               pyramid = ppArray[index];
            }
            index++;
         }
